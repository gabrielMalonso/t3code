...
const plusMillis = (iso: string, millis: number) =>
  new Date(Date.parse(iso) + millis).toISOString();
...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("applies provider session.state.changed transitions directly", async () => {
    const harness = await createHarness();
    const waitingAt = "2099-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-seed-zombie-turn"),
        threadId: asThreadId("thread-1"),
        session: {
          threadId: asThreadId("thread-1"),
          status: "running",
  ...
  it("accepts claude turn lifecycle when seeded thread id is a synthetic placeholder", async () => {
    const harness = await createHarness();
    const seededAt = "2099-01-01T00:00:00.000Z";
  ...
  it("does not create assistant segments for whitespace-only buffered text at approval boundaries", async () => {
    ...
    const pausedAt = plusMillis(startedAt, 1_000);
  ...
  it("starts a new buffered assistant message segment after approval and completes without duplication", async () => {
    ...
    const startedAt = new Date().toISOString();
    const pausedAt = plusMillis(startedAt, 1_000);
    const resumedAt = plusMillis(startedAt, 2_000);
    const completedAt = plusMillis(startedAt, 3_000);
  ...
  it("maps runtime.error into errored session state", async () => {
    ...
    const thread = await waitForThread(
      harness.readModel,
      (entry) =>
        entry.session?.status === "error" && entry.session?.lastError === "runtime exploded",
    );
    expect(thread.session?.status).toBe("error");
    expect(thread.session?.activeTurnId).toBeNull();
  ...
  it("projects completed context compaction items into context compaction activities", async () => {
    const harness = await createHarness();
    const now = "2099-01-01T00:00:00.000Z";

    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-context-compaction-item-completed"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-1"),
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) =>
          activity.id === "evt-context-compaction-item-completed",
      ),
    );

    const activity = thread.activities.find(
      (candidate: ProviderRuntimeTestActivity) =>
        candidate.id === "evt-context-compaction-item-completed",
    );
  ...
  it("continues processing runtime events after a single event handler failure", async () => {
    ...
    const thread = await waitForThread(
      harness.readModel,
      (entry) =>
        entry.session?.status === "error" && entry.session?.lastError === "runtime still processed",
    );
    expect(thread.session?.status).toBe("error");
    expect(thread.session?.activeTurnId).toBeNull();
