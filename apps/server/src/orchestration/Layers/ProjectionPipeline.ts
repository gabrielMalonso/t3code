      ...
      switch (event.type) {
        case "thread.created":
          yield* projectionThreadRepository.upsert({
            ...
            bootstrapPhase: event.payload.bootstrapPhase ?? "ready",
            latestTurnId: null,
            createdAt: event.payload.createdAt,
            updatedAt: event.payload.updatedAt,
            archivedAt: null,
        ...
        case "thread.session-set": {
          ...
          if (Option.isSome(existingTurn)) {
            const nextState =
              ...
              existingTurn.value.checkpointTurnCount === null &&
              existingTurn.value.checkpointRef === null
                ? "running"
        ...
        case "thread.turn-interrupt-requested": {
          const turnId =
            ...
            (yield* projectionTurnRepository.listByThreadId({
              threadId: event.payload.threadId,
            })).findLast((turn) => turn.turnId !== null && turn.state === "running")?.turnId ??
            undefined;
          if (turnId === undefined) {
            return;
          }
