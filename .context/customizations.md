...
| Thread loop por thread                      | web custom        | `apps/web/src/t3code-custom/chat/ThreadLoopControl.tsx`, `apps/web/src/t3code-custom/hooks/useThreadLoopActions.ts`, `apps/server/src/orchestration/Layers/ThreadLoopScheduler.ts`                                                                                                                                                                                                                                                                         | `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/chat/ChatComposer.tsx`, `apps/server/src/orchestration/*`, `apps/server/src/orchestration/Layers/ProjectionSnapshotQuery.ts`, `packages/contracts/src/orchestration.ts`                                                        | contracts e scheduler sao hotspot real; ler read model por `ProjectionSnapshotQuery`, nao ressuscitar metodo no engine                          |
...
- Regra pos VCS/projection:
  - `OrchestrationEngine` nao e fonte de read model
  - scheduler e bootstrap devem ler projeção por `ProjectionSnapshotQuery`
  - se o upstream mexer em VCS/GitWorkflow, aceitar o fluxo novo e reaplicar so branch rename/phase custom
...
- Regra adicional:
  ...
  - se o upstream mexer em remote/VCS/source control, nao reconstruir discovery paralelo para skills; manter skills como overlay de Codex
