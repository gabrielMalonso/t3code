...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  fileReferences: Schema.optionalKey(
    Schema.Array(
      Schema.Struct({
        id: Schema.String,
        name: Schema.String,
...
interface ComposerDraftStoreState {
  ...
  addFileReference: (threadRef: ComposerThreadTarget, reference: ComposerFileReference) => void;
  addFileReferences: (threadRef: ComposerThreadTarget, references: ComposerFileReference[]) => void;
  removeFileReference: (threadRef: ComposerThreadTarget, referenceId: string) => void;
  setFileReferences: (threadRef: ComposerThreadTarget, references: ComposerFileReference[]) => void;
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadKeyOrId, draftValue] of Object.entries(
    ...
    const attachments = Array.isArray(draftCandidate.attachments)
      ...
      : [];
    const fileReferences = Array.isArray(draftCandidate.fileReferences)
      ? draftCandidate.fileReferences.flatMap((entry) => {
          if (!entry || typeof entry !== "object") {
            return [];
          }
          const candidate = entry as ComposerFileReference;
          if (
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadKey, draft] of Object.entries(state.draftsByThreadKey)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.fileReferences.length > 0 ? { fileReferences: draft.fileReferences } : {}),
      ...(draft.terminalContexts.length > 0
        ? {
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    fileReferences: [...(persistedDraft.fileReferences ?? [])],
    nonPersistedImageIds: [],
  ...
  persist(
    (setBase, get) => {
      ...
      return {
        ...
        addFileReference: (threadRef, reference) => {
          get().addFileReferences(threadRef, [reference]);
        },
        addFileReferences: (threadRef, references) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || references.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const existingIds = new Set(existing.fileReferences.map((reference) => reference.id));
            const existingDedupKeys = new Set(
              existing.fileReferences.map((reference) => composerFileReferenceDedupKey(reference)),
            );
            const accepted: ComposerFileReference[] = [];
            for (const reference of references) {
              const dedupKey = composerFileReferenceDedupKey(reference);
              if (existingIds.has(reference.id) || existingDedupKeys.has(dedupKey)) {
                continue;
              }
            ...
            return {
              draftsByThreadKey: {
                ...
                [threadKey]: {
                  ...existing,
                  fileReferences: [...existing.fileReferences, ...accepted],
                },
              },
            };
          });
        },
        removeFileReference: (threadRef, referenceId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || referenceId.length === 0) {
            return;
          }
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              fileReferences: current.fileReferences.filter(
        ...
        setFileReferences: (threadRef, references) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
