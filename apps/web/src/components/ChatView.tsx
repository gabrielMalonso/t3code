...
export default function ChatView(props: ChatViewProps) {
  ...
  const composerFileReferencesRef = useRef<ComposerFileReference[]>([]);
  ...
  }, [activePlan?.turnId, showPlanSidebar, sidebarProposedPlan?.turnId]);
  ...
  useEffect(() => {
    ...
    showScrollDebouncer.current.cancel();
    setShowScrollToBottom(false);
    if (showPlanSidebar && planSidebarOpenOnNextThreadRef.current) {
    ...
    planSidebarDismissedForTurnRef.current = null;
  }, [activeThread?.id, showPlanSidebar]);

  useEffect(() => {
    if (showPlanSidebar || !planSidebarOpen) return;
    setPlanSidebarOpen(false);
  }, [planSidebarOpen, showPlanSidebar]);
  ...
  const onSubmitPlanFollowUp = useCallback(
    async ({
      ...
      try {
        ...
        if (nextInteractionMode === "default" && showPlanSidebar && autoOpenPlanSidebar) {
          planSidebarDismissedForTurnRef.current = null;
          setPlanSidebarOpen(true);
        }
        sendInFlightRef.current = false;
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    await api.orchestration
      ...
      .then(() => {
        // Signal that the plan sidebar should open on the new thread only when both
        // the local visibility gate and the upstream auto-open behavior are enabled.
        planSidebarOpenOnNextThreadRef.current = showPlanSidebar && autoOpenPlanSidebar;
        return navigate({
          to: "/$environmentId/$threadId",
          params: {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className={cn("px-3 pt-1.5 sm:px-5 sm:pt-2", isGitRepo ? "pb-1" : "pb-3 sm:pb-4")}>
            <ChatComposer
              ...
              composerFileReferencesRef={composerFileReferencesRef}
        ...
        {/* Plan sidebar */}
        {showPlanSidebar && planSidebarOpen && !shouldUsePlanSidebarSheet ? (
          <PlanSidebar
      ...
      {showPlanSidebar && shouldUsePlanSidebarSheet ? (
