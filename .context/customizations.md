...
| ------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| File references por path                    | web custom        | `apps/web/src/t3code-custom/file-references/*`, `apps/web/src/t3code-custom/hooks/useComposerFileReferenceSend.ts`, `apps/web/src/t3code-custom/hooks/useComposerPasteFileReference.ts`, `apps/web/src/t3code-custom/chat/{ComposerFileReferencesSlot,UserMessageFileReferencesSlot}.tsx`                                                                                                                                                              | `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/chat/ChatComposer.tsx`, `apps/web/src/components/chat/MessagesTimeline.tsx`, `apps/web/src/components/chat/PierreEntryIcon.tsx`, `apps/web/src/composerDraftStore.ts`, `apps/web/src/historyBootstrap.ts`, `apps/web/src/pierre-icons.ts`                                                                                                                | preservar o parser e a serializacao local; no core, manter so adaptadores pequenos; usar Pierre icons upstream, nao ressuscitar `vscode-icons`   |
...
- Perimetro local:
  ...
  - `apps/web/src/t3code-custom/chat/ComposerFileReferencesSlot.tsx`
  - `apps/web/src/t3code-custom/chat/UserMessageFileReferencesSlot.tsx`
- Adaptadores do core:
...
- Regra pos Pierre icons:
  - slots custom devem consumir `PierreEntryIcon` e `~/pierre-icons`
  - nao restaurar `VscodeEntryIcon`, `vscode-icons.ts` ou manifestos `vscode-icons-*`; isso agora e custom arqueologico
...
- Decisao de repositorio:
  ...
  - manter o Capacitor em `apps/mobile-capacitor/*` para deixar `apps/mobile/*` livre para o app React Native do upstream
...
- Hotspots compartilhados:
  ...
  - `pnpm-lock.yaml`
  - `pnpm-workspace.yaml`
- Perimetro upstream reservado:
  - `apps/mobile/*`
  - workflows/scripts/patches explicitamente ligados ao app React Native upstream
- Invariantes:
  ...
  - quando `ServerConfig.devUrl` restringir CORS ao dev UI, `http://localhost` e `capacitor://localhost` precisam continuar permitidos para o app Capacitor emitir bearer session e `wsTicket`
...
- Regra adicional de conflito:
  ...
  - se o upstream criar suporte mobile nativo equivalente, aceitar `apps/mobile/*` como upstream-puro e comparar depois se algum diferencial Android/Capacitor/Tailscale deve continuar em `apps/mobile-capacitor/*`
  - em syncs futuros, nao congelar `apps/mobile/*` para preservar Capacitor; preservar `apps/mobile-capacitor/*`, `apps/web/src/mobile/*` e docs mobile do donor
...
- Areas upstream relacionadas que nao devem virar fork paralelo:
...
- Regra pos Browser Preview upstream:
  - browser preview, webview, right panel e preview automation pertencem ao upstream
  - o fork nao deve recriar picking/browser automation dentro de `t3code-custom`
  - o diferencial local e o point-and-shoot externo via extensao/bridge; qualquer encaixe deve ser uma camada fina de intake para o composer
- Regra de conflito:
