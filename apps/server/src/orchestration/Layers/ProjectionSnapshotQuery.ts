      ...
      .withTransaction(
        Effect.gen(function* () {
          ...
          const projects: ReadonlyArray<OrchestrationProject> = projectRows.map((row) => {
            const repositoryIdentity = repositoryIdentities.get(row.projectId) ?? null;
            return {
              id: row.projectId,
              title: row.title,
              workspaceRoot: row.workspaceRoot,
        ...
        Effect.flatMap(
          ([
            ...
            Effect.sync(() => {
              ...
              for (let index = 0; index < threadLoopRows.length; index += 1) {
                const row = threadLoopRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
              ...
              for (let index = 0; index < sessionRows.length; index += 1) {
                ...
                sessionByThread.set(
                  row.threadId,
                  sanitizeBootSessionState(mapSessionRow(row), processStartedAt),
                );
              }
              ...
              for (const [threadId, latestTurn] of latestTurnByThread.entries()) {
                latestTurnByThread.set(
                  threadId,
                  sanitizeBootLatestTurnState(
                    latestTurn,
                    sessionByThread.get(threadId) ?? null,
                    processStartedAt,
                  ),
                );
              }
              ...
              for (let index = 0; index < threadLoopRows.length; index += 1) {
                ...
                loopsByThread.set(row.threadId, mapThreadLoopRow(row));
              }
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        worktreePath: threadRow.value.worktreePath,
        bootstrapPhase: threadRow.value.bootstrapPhase,
    ...
    Effect.gen(function* () {
      const [
        ...
        getThreadLoopRowByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:getThreadLoop:query",
              "ProjectionSnapshotQuery.getThreadDetailById:getThreadLoop:decodeRow",
            ),
          ),
        ),
      ]);
      ...
      const session = Option.isSome(sessionRow)
        ? sanitizeBootSessionState(mapSessionRow(sessionRow.value), processStartedAt)
        : null;
      const latestTurn = Option.isSome(latestTurnRow)
        ? sanitizeBootLatestTurnState(mapLatestTurn(latestTurnRow.value), session, processStartedAt)
        : null;
      ...
      const thread = {
        ...
        session,
        loop: Option.isSome(threadLoopRow) ? mapThreadLoopRow(threadLoopRow.value) : null,
      };
