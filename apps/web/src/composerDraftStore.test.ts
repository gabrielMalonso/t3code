...
describe("composerDraftStore clearComposerContent", () => {
  ...
  it("clears file references when clearing composer content", () => {
    useComposerDraftStore.getState().addFileReference(threadRef, {
      id: "file-ref-1",
      name: "plan.md",
...
describe("composerDraftStore file references", () => {
  const threadId = ThreadId.make("thread-file-refs");
  const threadRef = scopeThreadRef(TEST_ENVIRONMENT_ID, threadId);
  ...
  it("deduplicates file references by normalized path", () => {
    useComposerDraftStore.getState().addFileReferences(threadRef, [
      {
        id: "ref-1",
        name: "plan.md",
    ...
    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.fileReferences).toEqual([
      {
        id: "ref-1",
        name: "plan.md",
        path: "/tmp/project/docs/plan.md",
