        ...
        return startup
          .enqueueCommand(
            Effect.gen(function* () {
              ...
              if (guardedCommand.type === "thread.turn.start" && guardedCommand.bootstrap) {
                return yield* dispatchBootstrapTurnStart(guardedCommand);
              }
      ...
      const sanitizeClientThreadMetaUpdateCommand = (
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            !isBootstrapPhaseActive(thread.value.bootstrapPhase ?? "ready")
          ) {
            return command;
          }
          ...
          yield* Effect.logDebug("ignoring client thread meta bootstrap update", {
            threadId: command.threadId,
            bootstrapPhase: thread.value.bootstrapPhase,
          });
          ...
          Effect.catchCause((cause) =>
            Effect.logWarning("failed to sanitize client thread meta update", {
              threadId: command.threadId,
              cause: Cause.pretty(cause),
            }).pipe(Effect.as(command)),
          ),
        );
