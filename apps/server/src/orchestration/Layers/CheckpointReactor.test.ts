...
describe("CheckpointReactor", () => {
  ...
  it("still captures turn completion after activeTurnId is sanitized before completion", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-running-before-sanitize"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
    ...
    harness.provider.emit({
      type: "turn.started",
      eventId: EventId.make("evt-turn-started-sanitized-before-checkpoint"),
      provider: ProviderDriverKind.make("codex"),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-ready-after-sanitize"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    harness.provider.emit({
      type: "turn.completed",
      eventId: EventId.make("evt-turn-completed-sanitized-before-checkpoint"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: new Date().toISOString(),
      threadId: ThreadId.make("thread-1"),
      turnId: asTurnId("turn-sanitized-before-checkpoint"),
