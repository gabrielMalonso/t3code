...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const servedRoutesWithDiagnosticsLayer = servedRoutesBaseLayer.pipe(
      Layer.provide(
    ...
    const servedRoutesLayer = servedRoutesWithDiagnosticsLayer.pipe(
    ...
    yield* Layer.build(appLayer).pipe(Effect.orDie);
    return config;
  });
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("allows Capacitor mobile auth preflights when a dev URL is configured", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest({
