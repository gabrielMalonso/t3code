...
export default function ChatView(props: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    if (!activeThreadRef) return;
    useRightPanelStore.getState().close(activeThreadRef);
  }, [activeThreadRef, planSidebarOpen, showPlanSidebar]);
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    await (async () => {
      ...
      const resolvedTitleSeed =
        titleSeed === "New thread" && composerElementContextsSnapshot.length > 0
          ? formatElementContextLabel(composerElementContextsSnapshot[0]!)
          : titleSeed;
      const title = truncate(resolvedTitleSeed);
